...
bool find(CRC::Flags flags, CRC::Flags lookingFor) {
...
uint16_t DAC::transfer(
  ...
  //
  // Revised stability criterion: actually zeroes in Application::setupSPI()
  //
  // FSDO = 1, SPI_MODE2
  // CRC on: 26 Mbps okay
  // CRC off: 17 Mbps okay
  //
  // FSDO = 1, SPI_MODE2, delayMicroseconds(1) after SPI.endTransaction();
  // CRC on: 34 Mbps okay
  // CRC off: 34 Mbps okay
  //
  ...
  if (IC::Validation::enableCRC && find(flags, CRC::Flags::MISO_VALIDITY)) {
  ...
  uint16_t output = 0;
  if (find(flags, CRC::Flags::MISO_VALIDITY)) {
